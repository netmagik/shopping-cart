...
const cart = (props) => {
    ...
    return (
            ...
            <div><Button className="m-3" variant="outline-dark" disabled>Clear Shopping Cart</Button></div>
            <div className="mt-6 mb-4">{props.products.map((prod) => (
                <div key={prod.id} className="mt-4 mb-2 bg-light border pb-4">
                    <h5 className="mt-3">{prod.name}</h5>
                    <div className="pb-2"><strong>{prod.quantity} x ${prod.price}</strong></div>
                        <Button className="mr-2" variant="outline-dark" disabled>Change Quantity</Button>
